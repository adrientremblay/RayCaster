# 'Nightmare' Bible  
- Working Title
- By Adrien

## Executive Summary

### Concept
Nightmare (working title) is a horror, story-heavy, walking simulator game made using a
ray-casting graphics. The game will be nothing revolutionary gameplay-wise but will seek to
differentiate with its ambience and story.

### Target Audience
The target audience are people who are into horror games such as *Amnesia the Dark Descent*
that have a strong story aspect. Also the game would aim to attract people into interactive
fiction games like *Life is Strange* and *The Walking Dead*. Additionally another game I aim
to emulate is the walking-sim story game 'Dear Esther'.

## World
The entire game shall take place in a dream. But I want to avoid the whole "He woke up and it
was all a dream!" trope, so it should be made clear early to the player that they are in a
dream.

### Characters

### Setting

### Story

## Gameplay

### Levels

## Technical Details

### Art
The game use ray-casting to render the world in a pseudo 3D first person view. As such, it
makes the most sense to me to use pixelart for the textures. As the setting is in a dream,
the textures should have a dreamy vibe to them. Also a lot of the textures will be very dark
in colour to signify areas of the players dream that are more abstract.

### Graphics

### Audio

## Major Features List